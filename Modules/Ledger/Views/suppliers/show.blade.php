...
<div class="d-flex justify-content-between align-items-start mb-4 flex-wrap gap-2">
    <div class="d-flex align-items-center gap-3">
        @if ($supplier->photo)
            <img src="{{ asset('storage/' . $supplier->photo) }}" class="rounded-circle" style="width:56px;height:56px;object-fit:cover;">
        @else
        ...
        <div>
            ...
            <div class="text-muted small">
                {{ $supplier->mobile ?? '—' }} @if($supplier->city) · {{ $supplier->city }} @endif
            </div>
        </div>
    </div>
    <!-- <div class="d-flex gap-2">
        <a href="{{ route('ledger.suppliers.index') }}" class="btn btn-outline-secondary">
            <i class="bi bi-arrow-left"></i> {{ __('Back') }}
        </a>
        @can('easykhata.manage-transactions')
            <a href="{{ route('ledger.transactions.index',['supplier_id' => $supplier->id, 'standalone' => 1]) }}" class="btn btn-primary">
    ...
    <div class="d-flex gap-2">
        ...
        <a href="{{ route('ledger.suppliers.statement.pdf', $supplier) }}" class="btn btn-outline-danger" target="_blank">
            <i class="bi bi-file-earmark-pdf"></i> {{ __('Download PDF') }}
        </a>
        @can('easykhata.manage-transactions')
            <a href="{{ route('ledger.transactions.index', ['supplier_id' => $supplier->id]) }}" class="btn btn-primary">
                <i class="bi bi-plus-lg"></i> {{ __('New Transaction') }}
            </a>
        @endcan
    </div>
    ...
    <div class="col-6 col-md-3">
        <div class="ledger-stat-card p-3 h-100">
            ...
            <div class="small text-muted">
                {{ $balance > 0 ? __('Payable (they owe you)') : ($balance < 0 ? __('Credit (you owe them)') : __('Settled')) }}
            </div>
        </div>
    </div>
    <div class="col-6 col-md-3">
        <div class="ledger-stat-card p-3 h-100">
            ...
            <div class="h5 mb-0">{{ formatCurrency($supplier->opening_balance) }}</div>
        </div>
    </div>
    <div class="col-6 col-md-3">
        <div class="ledger-stat-card p-3 h-100">
            <div class="text-muted small">{{ __('Total Debit') }} ({{ __('You Owe Amount') }})</div>
            <div class="h5 mb-0 text-success">{{ formatCurrency($totalDebit) }}</div>
        </div>
    </div>
    <div class="col-6 col-md-3">
        <div class="ledger-stat-card p-3 h-100">
            <div class="text-muted small">{{ __('Total Credit') }} ({{ __('You Paid Amount') }})</div>
...
@if ($supplier->notes)
    <div class="alert alert-light border mb-4">
        <i class="bi bi-sticky"></i> {{ $supplier->notes }}
    </div>
@endif
                    ...
                    <tr>
                        ...
                        <td data-label="{{ __('Type') }}">
                            <span class="badge ledger-badge-{{ $tx->type }}">
                                ...
                                ({{ $tx->type === 'debit' ? __('You Owe') : __('You Paid') }})
                            </span>
                        </td>
                        <td data-label="{{ __('Reference') }}">{{ $tx->reference_no ?? '—' }}</td>
                        <td data-label="{{ __('Method') }}">{{ $tx->paymentMethod?->name ?? '—' }}</td>
                        <td data-label="{{ __('Debit') }}" class="text-end text-success">
                            {{ $tx->type === 'debit' ? formatCurrency($tx->amount) : '—' }}
                        </td>
                        <td data-label="{{ __('Credit') }}" class="text-end text-danger">
                            {{ in_array($tx->type, ['credit', 'opening_balance']) ? formatCurrency($tx->amount) : '—' }}
                        </td>
                    </tr>
                @empty
                    <tr class="ledger-row-empty">
                        <td colspan="6">
                            @include('ledger::partials.empty-state', [
                                'icon' => 'bi-receipt',
                                'title' => __('No transactions yet for this supplier'),
                            ])
                        </td>
                    </tr>
                @endforelse
