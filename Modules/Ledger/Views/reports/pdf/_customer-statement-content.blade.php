<table style="width:100%; margin-bottom: 14px;">
    <tr>
        <td style="width:33%;">
            ...
            <span style="font-size:13px; color:{{ $balance > 0 ? '#16A34A' : ($balance < 0 ? '#DC2626' : '#1e293b') }};">
                {{ formatCurrency(abs($balance)) }}
                {{ $balance > 0 ? '('.__('Receivable').')' : ($balance < 0 ? '('.__('Credit').')' : '') }}
            </span>
        </td>
        <td style="width:33%;">
...
<table class="data-table">
    ...
    <tbody>
        @php $running = $customer->opening_balance; @endphp
        @forelse ($transactions as $tx)
            @php
                $debit = in_array($tx->type, ['debit', 'opening_balance']) ? $tx->amount : 0;
